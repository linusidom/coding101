    ...
    try {
        ...
        if(user){
            ...
            if(comapredPassword){
                ...
                const token = await jwt.sign( {id:user._id}, dev.JWT_SECRET, {expiresIn:'1d'})
...
const userVerifyView = async (req, res) => {
    if(req.user){
        ...
        res.status(400).json({message:'User Verify: This should not fire'})
    }
}
...
const userPasswordResetRequestView = async (req, res) => {
    try {
        ...
        if(user){
            const token = await jwt.sign( {id:user._id}, dev.JWT_SECRET, {expiresIn:'1h'})
...
const userPasswordResetFormView = async (req, res) => {
    try {
        ...
        if(req.body.token){
            ...
            const verified = await jwt.verify(req.body.token, dev.JWT_SECRET)
